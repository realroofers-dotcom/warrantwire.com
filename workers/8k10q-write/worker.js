/* ============================================================================
   8K10Q-WRITE  —  Cloudflare Worker
   The writing side for 8K10Q: a login, a place to write, and the
   published pages.
   ...
   BINDINGS   DB         D1  → k8k10q_writing (this site's own)
...
const SITES = {
  '8k10q': {
    key: '8k10q', host: '8k10q.com',
    name: '8K10Q', url: 'https://8k10q.com',
    dark: true,
    tagline: "We read. It's never as it seems.",
    nav: [['/', 'What we read'], ['/writing', 'Writing'], ['/army.html', 'The Army'],
          ['https://warrantwire.com', 'Warrant Wire'], ['https://triggeredshort.com', 'Triggered Short']],
    blurb: 'One filing, read in plain English, with the sections that matter marked. '
         + 'Every figure points at the document it came from.',
    contact: 'sales@8k10q.com'
  }
};
function siteOf(u) {
...
async function setup(env) {
  ...
  try { await env.DB.prepare(
    "ALTER TABLE w_pieces ADD COLUMN site TEXT DEFAULT '8k10q'").run(); } catch (e) {}
  await env.DB.prepare(
  ...
  async fetch(req, env) {
    ...
    if (!S) {
      if (p === '/writing' || p.startsWith('/writing/'))
        return Response.redirect(SITES['8k10q'].url + p + u.search, 301);
      S = SITES['8k10q'];                /* the desk falls back to the house site */
    }
    const SITE = S.url;
    try {
      /* ---------- public ---------- */
...
async function list(env, S) {
  const rows = (await env.DB.prepare(
    `SELECT p.slug,p.title,p.standfirst,p.section,p.published,p.image,p.image_alt,p.words,
      ...
      WHERE p.status='published' AND COALESCE(p.site,'8k10q')=?
  ...
  const r = await env.DB.prepare(
    `SELECT p.*, w.name author_name, w.role, w.city, w.bio, w.photo, w.disclosure author_disclosure, w.kind
      ...
      WHERE p.slug=? AND p.status='published'
        AND COALESCE(p.site,'8k10q')=?`).bind(slug, S.key).first();
...
async function feed(env, S) {
  ...
  const rows = (await env.DB.prepare(
    `SELECT slug,title,standfirst,published FROM w_pieces WHERE status='published'
       AND COALESCE(site,'8k10q')=? ORDER BY published DESC LIMIT 50`)
...
async function sitemap(env, S) {
  ...
  const rows = (await env.DB.prepare(
    `SELECT slug, published, updated FROM w_pieces WHERE status='published'
       AND COALESCE(site,'8k10q')=?`).bind(S.key).all()).results || [];
  const x = `<?xml version="1.0" encoding="UTF-8"?>
...
<div class="wrap">
  ...
  <div class="main">
    ...
    <div id="pane-w">
      ...
      <p class="hint" style="margin:0 0 16px"><b>Publishing to 8K10Q</b> &mdash; https://8k10q.com/writing</p>
